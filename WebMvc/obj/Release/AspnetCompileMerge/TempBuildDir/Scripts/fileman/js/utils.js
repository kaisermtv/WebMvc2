...
RoxyUtils.LoadLang = function(){
  ...
  if(RoxyFilemanConf.LANG && RoxyFilemanConf.LANG.toLowerCase() == 'auto'){
    if(RoxyUtils.GetUrlParam('langCode')){
        langUrl = fileman_base + 'lang/' + RoxyUtils.GetUrlParam('langCode').substr(0, 2).toLowerCase() + '.json';
    }
    else {
      var language = window.navigator.userLanguage || window.navigator.language;
        langUrl = fileman_base + 'lang/' + language.substr(0, 2) + '.json';
    }
    ...
    if(RoxyFilemanConf.LANG){
        langUrl = fileman_base + 'lang/' + RoxyFilemanConf.LANG.substr(0, 2).toLowerCase() + '.json';
      if(!RoxyUtils.FileExists(langUrl))
        langUrl = '';
      }
  }
  if(!langUrl)
      langUrl = fileman_base + 'lang/en.json';
