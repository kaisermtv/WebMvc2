...
function dragFileOver(){
    $(this).children('img.dir').attr('src', fileman_base + 'images/folder-green.png');
}
function dragFileOut(){
    $('#pnlDirList').find('img.dir').attr('src', fileman_base + 'images/folder.png');
}
...
function makeDragDir(e) {
    ...
    var path = $(e.target).attr('data-path') ? $(e.target).attr('data-path') : $(e.target).closest('li').attr('data-path');
    var f = new Directory(name, path);
    return '<div class="pnlDragDir" data-path="' + f.fullPath + '"><img src="' + fileman_base + 'images/folder.png" align="absmiddle">&nbsp;'+f.name+'</div>';
}
...
function switchView(t){
  ...
  if(t == 'thumb'){
      $('#pnlFileList .icon').attr('src', fileman_base + 'images/blank.gif');
...
function setFile(){
  ...
  switch(getFilemanIntegration()){
      ...
      self.close();
          break;
      case 'buttoninject':
          var output = RoxyUtils.GetUrlParam('output');
          var imgshow = RoxyUtils.GetUrlParam('imgshow');
          var callback = RoxyUtils.GetUrlParam('callback');
          
          if (output && output != "") window.opener.document.getElementById(output).value = f.fullPath;
          if (imgshow && imgshow != "") window.opener.document.getElementById(imgshow).src = f.fullPath;

          //alert("window.opener." + decodeURI(callback) + "(f.fullPath);");
          if (callback && callback != "") {
              eval("window.opener." + decodeURI(callback) + "(f.fullPath);");
          }
