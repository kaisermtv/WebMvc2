...
*/
function Directory(name,fullPath, numDirs, numFiles, allowUpload){
  if(!fullPath) fullPath = '';
  this.fullPath = fullPath;
    this.allowUpload = allowUpload;
    if (!name || name == '') this.name = RoxyUtils.GetFilename(fullPath);
    else this.name = name;
    //alert(fullPath);
  if(!this.name)
        this.name = 'My files';
  ...
  this.SetEvents = function(){
    ...
    this.GetHtml = function () {
        var html = '<li data-name="' + this.name + '" data-path="' + this.fullPath + '" data-dirs="' + this.dirs + '" data-files="' + this.files + '" data-upload="' + this.allowUpload + '" class="directory">';
      html += '<div><img src="' + fileman_base +'images/'+(this.dirs > 0?'dir-plus.png':'blank.gif')+'" class="dirPlus" width="9" height="9">';
      html += '<img src="' + fileman_base +'images/folder.png" class="dir"><span class="name">'+this.name+' ('+this.files+')</span></div>';
     html += '</li>';
  ...
  this.Select = function(selectedFile){
    ...
    $('#pnlDirList li[data-path!="'+this.fullPath+'"] > div').removeClass('selected');
      el.children('img.dir').prop('src', fileman_base + 'images/folder.png');
    this.SetStatusBar();
  ...
  this.SetOpened = function(){
    ...
    if(li.find('li').length < 1)
        li.children('div').children('.dirPlus').prop('src', fileman_base +  'images/blank.gif');
    else if(this.IsExpanded())
        li.children('div').children('.dirPlus').prop('src', fileman_base +  'images/dir-minus.png');
    else
        li.children('div').children('.dirPlus').prop('src', fileman_base +  'images/dir-plus.png');
  };
    ...
    $.ajax({
        ...
        success: function (dirs) {
            ...
            for(i = 0; i < dirs.length; i++){
                var d = new Directory(dirs[i].Name,dirs[i].Path, dirs[i].CountFoder, dirs[i].CountFile, dirs[i].AllowUpload);
              d.Show();
            }
            $('#pnlLoadingDirs').hide();
            $('#pnlDirList').show();
...
Directory.Parse = function (path) {
  ...
  var ret = false;
    ...
    if (li.length > 0)
      ...
      ret = new Directory(li.attr('data-name'),li.attr('data-path'), li.attr('data-dirs'), li.attr('data-files'), li.attr('data-upload'));
