...
namespace WebMvc.Application.Extensions
{
    public static class Extensions
    {
        ...
        {
            string ret = ServiceFactory.Get<SettingsService>().GetSetting(key);
            if (ret == null) ret = "";
            return ret;
        }
        ...
        {
            var themeName = Settings(helper, "Theme");// ServiceFactory.Get<ISettingsService>().GetSetting("Theme");
            if (themeName.IsNullEmpty()) return "~/";
            return string.Concat(SiteConstants.Instance.ThemeRootFolder, themeName);
        }

        public static string ThemeUrl(this HtmlHelper helper)
        {
            return ThemePath(helper).Substring(1);
        }

		public static string ThemeUrl(this HtmlHelper helper,string text)
		{
			return ThemePath(helper).Substring(1) + text;
		}
		...
		public static MembershipUser CurrentMember(this HtmlHelper helper)
        {
            ...
            {
                return ServiceFactory.Get<MembershipService>().GetUser(System.Web.HttpContext.Current.User.Identity.Name);
            }
            return null;
        }
